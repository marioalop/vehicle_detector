...
from mongo_queries_manager import MongoDBQueriesManager
...
def mqmfork(string_query: str,
    ...
    for arg in args:
        ...
        if arg.startswith('sort='):
            ...
            key_value = mongodb_queries_mgr.filter_logic(filter_params=arg)
            for k in key_value.keys():
                if k in mongodb_query['filter'].keys():
                    mongodb_query['filter'][k].update(key_value[k])
                else:
                    mongodb_query['filter'] = {**mongodb_query['filter'], **key_value}
